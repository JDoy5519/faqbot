...
export async function POST(req: NextRequest) {
  try {
    ...
    // 3) Call the model (or use a free deterministic fallback)
let answer = "";
try {
  if (USE_FAKE_ANSWERS) {
    // Cheap: stitch together the top chunks so you can test the full flow for free
    const preview = matches.map((m, i) => `• ${m.content}`).join("\n");
    answer =
      matches.length > 0
